...
//-----------------------------------------------------------------------------
{
    ...
    if (name.contains(':'))
    {
        _timerName = name.after(":");
        _fatherName = name.before(":");
    }
    else
    {
    ...
    for (short tim = 0; tim < timers.size(); tim++)
    {
        ...
        if (!t->getFlag())
        {
            ...
            if (father != "")
            {
                ...
                for (short tim2 = 0; tim2 < timers.size(); tim2++)
                {
                    ...
                    if (!t2->getFlag() && t2->getFather() == t->getFather())
                    {
                        ...
                        _stream << "\n"
                                << t2->getName() << "\n";
                        _stream << "cpu-time: " << conv(t2->getTotal()) << "("<<t2->getTotal()<<") - " << (int((t2->getTotal() / fatherTime) * 10000)) / 100.0 << "%\n";
            ...
            else
            {
                t->setFlag(true);
                _stream << t->getName() << "\n";
                _stream << "cpu-time: " << conv(t->getTotal()) << "("<<t->getTotal()<<")\n";
